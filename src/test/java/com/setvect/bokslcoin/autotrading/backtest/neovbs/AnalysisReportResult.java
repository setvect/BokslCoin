...
import com.setvect.bokslcoin.autotrading.backtest.mabs.analysis.AnalysisMultiCondition;
...
public class AnalysisReportResult {
     ...
     * 종목별 매매 조건
     */
    private List<NeoVbsConditionEntity> conditionList;
    /**
     * 매매 이력
     */
    private List<NeoVbsTradeReportItem> tradeHistory;
    /**
     * 전체 수익 결과
     */
    ...
    public Set<String> getMarkets() {
        return conditionList.stream().map(NeoVbsConditionEntity::getMarket).collect(Collectors.toSet());
    }
    ...
    public Set<Integer> getMabsConditionIds() {
        return conditionList.stream().map(NeoVbsConditionEntity::getVbsConditionSeq).collect(Collectors.toSet());
    }
