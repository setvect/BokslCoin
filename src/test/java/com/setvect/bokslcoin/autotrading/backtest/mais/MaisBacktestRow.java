...
public class MaisBacktestRow {
    ...
    public MaisBacktestRow(Candle candle) {
        this.candle = candle;
    }
    ...
    public String toString() {
        ...
        return String.format("날짜(KST): %s, 날짜(UTC): %s, 시가: %,.0f, 고가:%,.0f, 저가:%,.0f, " +
                        "종가:%,.0f, 직전 종가:%,.0f, 단위 수익률: %,.2f%%, 이동평균: %,.0f, 최대 이동평균: %,.0f, 최소 이동평균: %,.0f, " +
                        "매매여부: %s, 매수 체결 가격: %,.0f, 최고수익률: %,.2f%%, 매도 체결 가격: %,.0f, 매도 이유: %s, " +
                ...
                candle.getTradePrice(), beforeTradePrice, getCandleYield() * 100, currentMa, maxMa, minMa,
                trade, bidPrice, highYield * 100, askPrice, askReason == null ? "" : askReason,
