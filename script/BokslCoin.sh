...
start() {
  ...
  nohup $JAVA -Dlogging.config=../conf/logback-spring.xml -Dspring.profiles.active=local -jar ../lib/BokslCoin-0.3.0.jar --spring.config.location=file:../conf/BokslCoin.yml 1> /dev/null 2>&1 &
  echo $! > ${RUNNING_PID}
...
console() {
  $JAVA -Dlogging.config=../conf/logback-spring.xml -Dspring.profiles.active=local -jar ../lib/BokslCoin-0.3.0.jar --spring.config.location=file:../conf/BokslCoin.yml
}
